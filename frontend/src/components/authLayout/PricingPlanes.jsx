...
export function PricingPlanes() {
  const { setUsers, setSelectedPlan } = useOrganization();
  const { setStep } = useAuth();

  const [users, setUsersLocal] = useState(1);
  const [selectedPlan, setSelectedPlanLocal] = useState(plans[0]);
      ...
      <div className="grid gap-4 text-center w-full">
        ...
        <div className="mb-4 flex items-center gap-2 w-full justify-center">
          ...
          <Input
            ...
            className="w-64"
            value={users}
            onChange={(e) => setUsersLocal(Number(e.target.value))}
          />
        </div>
      </div>
